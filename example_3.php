...
class ChildValues extends Values {
    public function createHistogram($start,$finish) {
        $dataPoints = array();
    foreach ($this->num as $number => $iterated_number) {
        $array2 = array("y" =>$iterated_number, "label" =>$number);
        $dataPoints[] = $array2;
    }
    return $dataPoints;
}
